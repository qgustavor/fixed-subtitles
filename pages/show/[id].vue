<template>
  <div class="dark:text-white text-center text-2xl py-20" v-if="!show">
    ...
    <div class="text-8xl mt-2">
      <Icon name="carbon:document-unknown" class="iconify inline-block" />
    </div>
  </div>
    ...
    <div class="w-full">
      ...
      <div v-if="selectedGroup">
        <div class="pb-2 lg:text-lg">
          <Icon name="carbon:language" class="iconify group-stat-icon mr-1" />
          {{ t('languages.' + selectedGroup.metadata.language, '?') }}
          <Icon name="carbon:categories" class="iconify group-stat-icon mx-1" />
          {{ t('file-count', selectedGroup.files.length) }}
          <Icon name="carbon:clean" class="iconify group-stat-icon mx-1" />
          ...
          <a
            ...
            target="_blank"
          >
            <Icon name="carbon:edit" class="iconify group-stat-icon mx-1" />
          ...
          <button @click="downloadFile()" v-if="selectedGroup.files.length > 1">
            <Icon name="carbon:download" class="iconify group-stat-icon mx-1" />
...
const show = computed(() => shows.find(e => e.slug === route.params.id))
...
const title = computed(() => show.value && show.value.metadata.title)
useHead({ title })
</script>
